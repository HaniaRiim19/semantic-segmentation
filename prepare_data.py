...
def prepare_data_inference(idx_order, args, sent2vec_model):
    original_x = []  # List to store the original x
    processed_x = []  # List to store the processed x

    # iterate over documents
    for doc in idx_order:
        doc_data = []  # List to store sentence data for this document
        data_path = args.data_path + doc + '.txt'

        if data_path != 'infer/data/.ipynb_checkpo.txt':
            with open(args.data_path + doc + '.txt') as fp:
                # iterate over sentences
                for sent in fp:
                    sent_x = sent.strip()
                    ...
                    if not args.pretrained:
                        ...
                        #print(sent_x)
                        sent_x_mapped = list(map(lambda x: args.word2idx[x] if x in args.word2idx else args.word2idx['<unk>'], sent_x.split()))
                        #print(sent_x_mapped)
                    else:
                        sent_x_mapped = sent2vec_model.embed_sentence(sent_x).flatten().tolist()[:args.emb_dim]

                    if sent_x_mapped != []:
                        # Store both the original sentence and its processed representation
                        sent_data = {
                            'original': sent_x,
                            'processed': sent_x_mapped
                        }
                        #print(sent_data)
                        doc_data.append(sent_data)
                        #print(doc_data)

        original_x.append([sent_data['original'] for sent_data in doc_data])
        processed_x.append([sent_data['processed'] for sent_data in doc_data])
