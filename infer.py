...
def main():
    ...
    original,x = prepare_data_inference(idx_order, args, sent2vec_model)
    if x[0]==[]:
      x = x[1:]
    ...
    x = [inner_list for inner_list in x if inner_list]
    original = [inner_list for inner_list in original if inner_list]
    #len(original)
    print("haha")
    print(original)
    print(x)
    ...
    for idx, doc in enumerate(idx_order):
      if idx < len(x):  # Check if the index is within the range of x
    ...
    with open(args.save_path, 'w') as fp:
        for sentence, label in zip(original, p):
            fp.write(f"{sentence}\t{label}\n")
